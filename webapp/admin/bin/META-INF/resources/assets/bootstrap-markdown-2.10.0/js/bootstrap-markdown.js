...
//添加公式 begin
      {
        ...
        data: [{
          ...
          callback: function(e){
            ...
            if (selected.length === 0) {
              // Give extra word
              chunk = e.__localize('Formula');
            } else {
              chunk = selected.text;
            }
            // transform selection and set the cursor into chunked text
            if (content.substr(selected.end, 1) === ')') {
              e.setSelection(selected.start - 43, selected.end + 1);
              e.replaceSelection(chunk);
              cursor = selected.start - 43;
            } else {
              e.replaceSelection('![公式名](http://latex.codecogs.com/png.latex?' + chunk + ')');
              cursor = selected.start + 43;
            }
            // Set the cursor
            e.setSelection(cursor, cursor + chunk.length);
